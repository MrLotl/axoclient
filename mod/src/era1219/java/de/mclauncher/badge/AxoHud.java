...
public final class AxoHud {
	...
	public static void tick() {
		...
		if (client.getWindow() == null)
			return;
		boolean down = InputUtil.isKeyPressed(client.getWindow(), MENU_KEY);
		if (down && !keyWasDown && client.currentScreen == null && client.player != null)
			client.setScreen(new HudScreen());
		keyWasDown = down;
		boolean fullbrightDown = InputUtil.isKeyPressed(client.getWindow(), FULLBRIGHT_KEY);
	...
	public static void render(DrawContext context) {
		MinecraftClient client = MinecraftClient.getInstance();
		// Seit 1.21.9 ist shouldShowDebugHud() auch dann wahr, wenn nur eine einzelne Debug-Anzeige
		// (z.B. Chunk-Grenzen mit F3+G) läuft; gemeint ist hier aber nur der sichtbare F3-Text.
		if (client.player == null || client.options.hudHidden || client.debugHudEntryList.isF3Enabled())
			return;
