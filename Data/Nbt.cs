...
/// <summary>Ein NBT-Compound: benannte Werte (string, int, long, NbtCompound, NbtList, ...).</summary>
public class NbtCompound : Dictionary<string, object>
{
    public T? Get<T>(string key) => TryGetValue(key, out var v) && v is T t ? t : default;
}

/// <summary>Eine NBT-Liste; alle Elemente haben denselben Tag-Typ.</summary>
...
/// <summary>
/// Minimaler Leser/Schreiber für Minecrafts NBT-Format (Big Endian), z.B. level.dat (gzip) und servers.dat (roh).
/// </summary>
public static class Nbt
{
    ...
    public static NbtCompound ReadFile(string path)
    {
        ...
        // gzip erkennt man an den ersten beiden Bytes 1F 8B
        ...
        ReadString(reader); // Name des Wurzel-Tags (meist leer)
